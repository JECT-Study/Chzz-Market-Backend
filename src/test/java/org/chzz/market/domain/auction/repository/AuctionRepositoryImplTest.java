...
class AuctionRepositoryImplTest {
    ...
    static void setUpOnce(@Autowired UserRepository userRepository,
        ...
        auction1 = Auction.builder().product(product1).status(AuctionStatus.PROCEEDING)
                .endDateTime(LocalDateTime.now().plusDays(1)).build();
        auction2 = Auction.builder().product(product2).status(AuctionStatus.PROCEEDING)
                .endDateTime(LocalDateTime.now().plusDays(1)).build();
        auction3 = Auction.builder().product(product3).status(AuctionStatus.PROCEEDING)
                .endDateTime(LocalDateTime.now().plusDays(1)).build();
        auction4 = Auction.builder().product(product4).status(AuctionStatus.CANCELLED)
                .endDateTime(LocalDateTime.now().plusDays(1)).build();
