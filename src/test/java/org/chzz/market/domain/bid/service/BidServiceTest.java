...
class BidServiceTest {
    ...
    void setUp() {
        ...
        product = Product.builder().id(1L).user(user).name("제품1").category(Category.FASHION).build();
        product2 = Product.builder().id(2L).user(user).name("제품2").category(Category.FASHION).build();
        product3 = Product.builder().id(3L).user(user).name("제품3").category(Category.FASHION).build();
        auction = Auction.builder().id(1L).product(product).minPrice(1000L).status(Status.PROCEEDING)
                .endDateTime(LocalDateTime.now().plusDays(1)).build();
        completeAuction = Auction.builder().id(2L).product(product2).minPrice(1000L).status(Status.COMPLETE)
                .endDateTime(LocalDateTime.now().minusDays(1)).build();
        endAuction = Auction.builder().id(3L).product(product3).minPrice(1000L).status(Status.PROCEEDING)
                .endDateTime(LocalDateTime.now().minusDays(1)).build();
    }
    ...
    public void firstBid_Success() throws Exception {
        ...
        //when
        bidService.createBid(bidCreateRequest, user2);

        //then
        Assertions.assertDoesNotThrow(() -> bidService.createBid(bidCreateRequest, user2));
    }
    ...
    public void ownerBid_ThrowsException() throws Exception {
        ...
        // when & then
        assertThatThrownBy(() -> bidService.createBid(bidCreateRequest, user))
    ...
    public void notProceeding_ThrowsException() throws Exception {
        ...
        //when & then
        assertThatThrownBy(() -> bidService.createBid(bidCreateRequest, user2))
