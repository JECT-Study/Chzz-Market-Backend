...
public enum NotificationType {
    AUCTION_SUCCESS("%s 경매가 낙찰되었습니다.", Values.AUCTION_SUCCESS) {
        ...
        public Notification createNotification(Long userId, NotificationEvent event) {
            return new AuctionSuccessNotification(userId, event.image(), event.message(), event.getAuctionId());
        }
    },
    AUCTION_FAILURE("%s 경매가 유찰되었습니다.", Values.AUCTION_FAILURE) {
        @Override
        public Notification createNotification(Long userId, NotificationEvent event) {
            return new AuctionFailureNotification(userId, event.image(), event.message());
        }
    },
    AUCTION_WINNER("축하합니다! 입찰에 참여한 %s 경매에 낙찰되었습니다.", Values.AUCTION_WINNER) {
        @Override
        public Notification createNotification(Long userId, NotificationEvent event) {
            return new AuctionWinnerNotification(userId, event.image(), event.message(), event.getAuctionId());
        }
    },
    AUCTION_NON_WINNER("입찰에 참여한 %s 경매에 낙찰되지 못했습니다.", Values.AUCTION_NON_WINNER) {
        @Override
        public Notification createNotification(Long userId, NotificationEvent event) {
            return new AuctionNonWinnerNotification(userId, event.image(), event.message());
        }
    },
    AUCTION_START("좋아요를 누른 %s 경매가 시작되었습니다.", Values.AUCTION_START) {
        @Override
        public Notification createNotification(Long userId, NotificationEvent event) {
            return new AuctionStartNotification(userId, event.image(), event.message(), event.getAuctionId());
        }
    },
    PRE_AUCTION_CANCELED("좋아요를 누른 %s 사전 경매가 판매자에 의해 취소되었습니다.", Values.PRE_AUCTION_CANCELED) {
        @Override
        public Notification createNotification(Long userId, NotificationEvent event) {
            return new PreAuctionCanceledNotification(userId, event.image(), event.message());
        }
    };
